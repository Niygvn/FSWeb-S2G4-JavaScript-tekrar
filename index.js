...
/* kodlar buraya */
ucebolunenlerintoplami = ucetambolunenler.reduce(function (total, item) {
  return total + item;
}, 0);
...
/* kodlar buraya */
besyuzdenkucuksayilar = [];
sayilar.filter(function (items) {
  if (items < 500) {
    besyuzdenkucuksayilar.push(items);
  }
});
...
// Find repeated numbers and their counts
let arr = Object.keys(sayac).filter(function (num) {
  return sayac[num] > 1;
});
tekraredensayilar = [];
